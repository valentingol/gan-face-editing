...
def project_images(images, save_intermediate=False):
    ...
    for iteration in pbar:
        if save_intermediate and (iteration + 1) % 100 == 0:
            ...
            with torch.no_grad():
                rec_images = generator(proj_styles, randomize_noise=False, input_is_style=True)[0]
...
if __name__ == "__main__":
    ...
    # NOTE configs : 'anycost-ffhq-config-f', 'anycost-ffhq-config-f-flexible', 'stylegan2-ffhq-config-f'
    ...
    parser.add_argument("--save_intermediate", action='store_true', help="save intermediate images every 100 iterations")
    # output directory to save images and latent inside
    parser.add_argument("--output_dir", type=str, default='./', help="output directory to save images and latent inside")
    ...
    projected_styles = project_images(imgs, save_intermediate=args.save_intermediate)
    ...
    for i, input_name in enumerate(args.files):
        ...
        import numpy as np
        path_latent = os.path.join(args.output_dir, 'projected_latents', basename)
