...
def train_boundary(latent_codes, scores, chosen_num_or_ratio=0.02,
                   split_ratio=0.7, invalid_value=None):
...
def project_boundary(primal, *args):
    ...
    *args :
    ...
    if len(args) == 1:
        cond = args[0]
        assert (len(cond.shape) == 2 and cond.shape[0] == 1
    ...
    if len(args) == 2:
        cond_1 = args[0]
        cond_2 = args[1]
        assert (len(cond_1.shape) == 2 and cond_1.shape[0] == 1
                and cond_1.shape[1] == primal.shape[1])
        assert (len(cond_2.shape) == 2 and cond_2.shape[0] == 1
...
def linear_interpolate(latent_code, boundary, start_distance=-3.0,
                       end_distance=3.0, steps=10):
    ...
    """
    assert (latent_code.shape[0] == 1 and boundary.shape[0] == 1
