    ...
    with tqdm(total=len(data_loader),
        ...
        for batch_idx, real_img in enumerate(data_loader):
            ...
            if args.d_reg_every > 0 and global_idx % args.d_reg_every == 0:
                ...
                discriminator.zero_grad()
                reg_d_loss = args.r1 / 2 * r1_loss * args.d_reg_every \
                    + 0 * real_pred[0]
                reg_d_loss.backward()
                d_optim.step()
            ...
            # reg G
            if args.g_reg_every > 0 and global_idx % args.g_reg_every == 0:
                ...
                weighted_path_loss = args.path_regularize * args.g_reg_every \
                    * path_loss
                # special trick to trigger sync gradient descent TODO:
                # do we need it here?
...
def validate(epoch):
    ...
    if hvd.rank() == 0:
        n = len(data_loader) * (epoch + 1) * args.batch_size * hvd.size()
        log_writer.add_scalar('Metrics/fid', fid, n)
    global best_fid
...
def measure_fid():
    ...
    n_res_to_eval = len(args.inception_path.split(','))
    ...
    # Just make sure they use different seed
    torch.manual_seed(int(time.time()) + hvd.rank() * 999)
    ...
    with torch.no_grad():
        for _ in tqdm(range(n_batch), desc='FID', disable=hvd.rank() != 0):
            ...
            for i_res in range(n_res_to_eval):
                ...
                feat = inception(img.clamp(-1., 1.))[0].view(
                    img.shape[0], -1
                    ).to('cpu')
    ...
    fid0 = hvd.broadcast(torch.tensor(fid_dict[args.resolution]).float(),
                         root_rank=0, name='fid').item()
...
if __name__ == "__main__":
    ...
    # cudnn benchmark will lead to bug when computing G regularization,
    # resulting in extremely slow computing
    ...
    if hvd.rank() == 0:  # measure flops and #param
        print(' * D Params: {:.2f}M'.format(
            sum([p.numel()for p in discriminator.parameters()]) / 1e6
            ))
        print(' * G Params: {:.2f}M'.format(
