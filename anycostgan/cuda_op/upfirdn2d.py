...
class UpFirDn2dBackward(Function):
    ...
    @staticmethod
    def forward(
            ctx, grad_output, kernel, grad_kernel, up, down, pad, g_pad,
            in_size, out_size
            ):
        ...
        grad_input = upfirdn2d_op.upfirdn2d(
                grad_output, grad_kernel, down_x, down_y, up_x, up_y, g_pad_x0,
    ...
    def backward(ctx, gradgrad_input):
        ...
        gradgrad_out = upfirdn2d_op.upfirdn2d(
                gradgrad_input, kernel, ctx.up_x, ctx.up_y, ctx.down_x,
                ctx.down_y, ctx.pad_x0, ctx.pad_x1, ctx.pad_y0, ctx.pad_y1,
                )
        gradgrad_out = gradgrad_out.view(
                ctx.in_size[0], ctx.in_size[1], ctx.out_size[0],
                ctx.out_size[1]
                )
...
class UpFirDn2d(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = UpFirDn2dBackward.apply(
                grad_output, kernel, grad_kernel, ctx.up, ctx.down, ctx.pad,
