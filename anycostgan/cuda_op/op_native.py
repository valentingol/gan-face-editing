...
def upfirdn2d_native(X, kernel, up_x, up_y, down_x, down_y, pad_x0, pad_x1,
    ...
    if up_y == 2:
        w = X.new_zeros(2, 2)
        w[0, 0] = 1
        out = F.conv_transpose2d(X, w.view(1, 1, 2, 2).repeat(
            chanels, 1, 1, 1), groups=chanels, stride=2)
    else:
        out = X

    out = F.pad(out, [pad_x0, pad_x1, pad_y0, pad_y1])
    out = F.conv2d(out, kernel.view(1, 1, kernel_h, kernel_w)
                   .repeat(chanels, 1, 1, 1), groups=chanels)
