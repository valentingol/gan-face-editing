...
class Generator(nn.Module):
    ...
    def __init__(self, resolution, style_dim=512, n_mlp=8,
                 ...
                 blur_kernel=(1, 3, 3, 1), lr_mlp=0.01, act_func='lrelu'):
...
class DiscriminatorMultiRes(nn.Module):
    ...
    def __init__(self, resolution, channel_multiplier=2, channel_max=512,
                 blur_kernel=(1, 3, 3, 1), act_func='lrelu', n_res=1,
                 modulate=False):
        """Initialize the discriminator."""
        super().__init__()
        ...
        for i in range(log_res, 2, -1):
            ...
            self.blocks.append(
                ResBlock(
                    in_channel, out_channel, blur_kernel, act_func=act_func,
                    modulate=modulate
                    and i in list(range(log_res, 2, -1))[-2:], g_arch_len=4 *
                    (log_res*2 - 2)))
            in_channel = out_channel
