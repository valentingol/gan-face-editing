...
from anycostgan.models.anycost_gan import (G_CHANNEL_CONFIG, ConstantInput,
                                           ModulatedConv2d)
...
def set_uniform_channel_ratio(model, ratio):
    ...
    channels = {k: min(channel_max, int(v * ratio * org_channel_mult))
...
def get_random_channel_config(full_channels, org_channel_mult,
    ...
    for full_c, bottom in zip(full_channels, bottom_line):
        valid_channel_configs = [a for a in CHANNEL_CONFIGS
                                 if a * full_c >= bottom]
